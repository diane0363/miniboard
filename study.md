...
// show-sql : true
    // 쿼리를 한 줄로 안 쓰고 예쁘게 정렬해서 보여주는 옵션 (콘솔창에서 JPA 코드가 의도한 대로 쿼리를 잘 만들어내고 있는지 확인)
    // 실제 서비스용 서버에서는 끄고, 대신 로깅 라이브러리 설정(logging.level.org.hibernate.SQL=debug)을 쓰는 것이 정석

// Comment.java
    // 팩토리 메서드 -> 인스턴스를 만들어서 돌려주는 것
    // Comment.ofRoot(...) — 인스턴스 없이 클래스에 직접 부른다. 이게 정적 팩토리의 기본 문법
    // 만약 인스턴스 메서드로 만들면? 해당 메서드를 호출하려면 이미 Comment 객체가 하나 있어야 한다. 
    // 객체를 만들려고 하는데, 그러려면 먼저 객체가 있어야 한다는 순환 
    // => 그래서 객체 없이 클래스 이름만으로 호출할 수 있는 static이 될 수밖에 없다.  

    // 팩토리 : 객체 생성을 한 곳에 가두고 이름을 붙이는 것
    // 생성(new)은 당연히 팩토리 안에서. 대신 호출하는 쪽(Service)은 new를 절대 안 쓴다. 
